...
async function run() {
  ...
  ok(
    '4. Hàng lỗi vào QUARANTINE + RMA, xuất 1 cuốn NEW thay thế',
    newAfter === newBefore - 1 && quarAfter - quarBefore === 1 && rma.length > 0,
  ...
  try {
    ...
    rolledBack = e.code === 'INSUFFICIENT_ATP' || e.code === 'INVALID_INPUT' || /Không đủ tồn|Giá trị sách/i.test(e.message);
  }
  ...
  await ReturnService.voidReturn(req8.returnId, 'ROLE_MANAGER', 'Test huy phieu', RCTX('manager-1', 'ROLE_MANAGER'), uniq('idem-void'));
