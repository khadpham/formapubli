...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: false, error: error.message || 'Lỗi lọc khách theo tag' }, { status: 400 });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const status = /Kế toán thuế/.test(error.message || '') ? 403 : 400;
    return NextResponse.json({ success: false, error: error.message || 'Lỗi gắn tag' }, { status });
  }
}
